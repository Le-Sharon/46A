...
public class BookTitle12
{
     ...
     */
    public BookTitle12(String bookTitle)
    {
    ...
    public String lastLetters()
    {
        ...
        for (int x = 0; x <= title.length()-1; x++)
        {
            ...
            if (!singleLetter.equals(" "))
            {
                singleWord = singleWord + title.charAt(x);
            }
            else if (singleLetter.equals(" "))
            {
                char lastChar = this.lastLetter(singleWord);
                each = each + lastChar;
                singleWord = "";
            }
            ...
            if (x == title.length()-1)
            {
                ...
                singleWord = title.substring(lastSpace);
                char lastChar = this.lastLetter(singleWord);
                each = each + lastChar;
            }
        }
