...
public class TemperatureTable
{
    ...
    public double columnAverage(int colIndex)
    {
        ...
        double count = integers.length;//count how many rows, totalRows = elements in col
        double sums = 0;
    ...
    public int largerValueInMaxRowMinColumn(int limit)
    {
        ...
        int largestSoFar = 0;
        boolean found = false;
        for (int x = 0; x < integers[0].length; x++) //total columm as indexes, collum cannot be negative cuz they're never empty
        {
            //now by rows, this will scan the entirety of each collum from collum 2 to 0
            for (int y = integers.length - 1; y >= 0; y--)
            {
                if (!found && integers[y][x] > limit)
                {
